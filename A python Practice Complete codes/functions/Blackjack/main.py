...
while not should_end:
    play_or_not = input("Do you want to play blackjack ?\n")
    if play_or_not == "yes":
        def deal_card():
        ...
        def calculate_score(list_of_cards):
            ...
            #     if item == 11 or item == 10:
            #         return 0
            user_score = 0
            if list_of_cards[0] == 11 and list_of_cards[1] == 10 or list_of_cards[1] == 11 and list_of_cards[0] ==10:
                return 0

            else:
                user_score = sum(list_of_cards)
                if user_score < 21:
                    user_choice = input("Do you want to add another card to user's cards?: ")
                    if user_choice == "yes":
                        user_cards.append(deal_card())
                        user_score = sum(list_of_cards)
                        print(user_cards)
                        if user_score > 21:
                            return "Burst😭 Score exceeded than 21\nYou loose!"
